...
| `/memory [pending\|approve\|reject\|approval\|phi]` | Review pending memory writes staged by the write-approval gate (`memory.write_approval`), toggle the gate, or use legacy Phi Memory shims when the optional `phi-memory` plugin is enabled. Prefer `/phi-memory status` and `/phi-memory compress --apply-safe`. See [Persistent Memory](/user-guide/features/memory). |
| `/phi-memory status\|review\|compress\|explain\|recall\|dashboard\|meta\|review-due\|schedule\|skills` | Optional Phi Memory plugin slash command for golden-ratio review/scoring, metadata sidecars, Fibonacci review scheduling, skill-candidate drafts, semantic compression proposals, opt-in semantic apply, and deterministic safe cleanup of built-in `MEMORY.md` / `USER.md`. Enable with `hermes plugins enable phi-memory`. |
...
| `/memory [pending\|approve\|reject\|approval\|phi]` | Review pending memory writes staged by the write-approval gate (`memory.write_approval`) — approve or reject them right in chat — toggle the gate with `/memory approval on\|off`, or use legacy `/memory phi ...` shims when the optional `phi-memory` plugin is enabled. Prefer `/phi-memory ...`; `/phi-memory compress --apply-safe` applies only deterministic safe cleanup with a backup. See [Persistent Memory](/user-guide/features/memory). |
| `/phi-memory status\|review\|compress\|explain\|recall\|dashboard\|meta\|review-due\|schedule\|skills` | Optional Phi Memory plugin command for metadata sidecars, review scheduling, session fallback recall, skill candidates, semantic proposals, opt-in semantic apply, and deterministic cleanup. `compress` is dry-run unless `--apply-safe` or the advanced `--apply-semantic` flag is used. `compress --semantic` is proposal-only. |
