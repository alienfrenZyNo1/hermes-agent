...
| `/phi-memory status\|review\|compress\|explain\|recall\|dashboard\|meta\|review-due\|schedule\|skills` | Optional Phi Memory plugin slash command for golden-ratio review/scoring, metadata sidecars, Fibonacci review scheduling, skill-candidate drafts, semantic compression proposals, and deterministic safe cleanup of built-in `MEMORY.md` / `USER.md`. Enable with `hermes plugins enable phi-memory`. |
