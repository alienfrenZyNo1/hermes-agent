...
| `/memory [pending\|approve\|reject\|approval\|phi]` | Review pending memory writes staged by the write-approval gate (`memory.write_approval`), toggle the gate, or run Phi Memory review commands such as `/memory phi status` and deterministic safe cleanup with `/memory phi compress --apply-safe`. See [Persistent Memory](/user-guide/features/memory). |
...
| `/memory [pending\|approve\|reject\|approval\|phi]` | Review pending memory writes staged by the write-approval gate (`memory.write_approval`) — approve or reject them right in chat — toggle the gate with `/memory approval on\|off`, or inspect Phi Memory with `/memory phi status\|review\|compress\|explain\|recall`; `/memory phi compress --apply-safe` applies only deterministic safe cleanup with a backup. See [Persistent Memory](/user-guide/features/memory). |
