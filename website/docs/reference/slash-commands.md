...
| `/phi-memory status\|review\|compress\|explain\|recall` | Optional Phi Memory plugin slash command for golden-ratio review/scoring and deterministic safe cleanup of built-in `MEMORY.md` / `USER.md`. Enable with `hermes plugins enable phi-memory`. |
...
| `/memory [pending\|approve\|reject\|approval\|phi]` | Review pending memory writes staged by the write-approval gate (`memory.write_approval`) — approve or reject them right in chat — toggle the gate with `/memory approval on\|off`, or use legacy `/memory phi ...` shims when the optional `phi-memory` plugin is enabled. Prefer `/phi-memory status\|review\|compress\|explain\|recall`; `/phi-memory compress --apply-safe` applies only deterministic safe cleanup with a backup. See [Persistent Memory](/user-guide/features/memory). |
| `/phi-memory status\|review\|compress\|explain\|recall` | Optional Phi Memory plugin command for golden-ratio review/scoring of built-in memory files; `compress` is dry-run unless `--apply-safe` is passed. |
